...
format:
  ...
  html:
    ...
    link-external-filter: '^(?:http:|https:)\/\/(?:www\.)?johnbaker\.io(?:\/|$)'
format-links:
  - html
  - format: typst
    text: PDF
...
3. **Mitigated bias** using attribute-appropriate techniques from all three mitigation families: threshold optimization (post-processing) for large groups (region, IMD, age), reweighting (pre-processing) for underrepresented groups (disability), and adversarial debiasing (in-processing) as a viable alternative for all attributes, reducing Equal Opportunity Difference to near-zero while maintaining AUC
...
- **Mitigation approach matters:** Post-processing (threshold optimization) works for well-represented groups; pre-processing (reweighting) is needed for minorities like students with disabilities (9% of data); in-processing (adversarial debiasing) provides a viable alternative across all group sizes
...
with open(DATA_DIR / 'fairness_results_with_ci.json', 'r') as f:
    fairness_results_ci = json.load(f)

with open(DATA_DIR / 'mitigation_results_adversarial_region.json', 'r') as f:
    mitigation_adversarial_region = json.load(f)

with open(DATA_DIR / 'mitigation_results_adversarial_imd_band.json', 'r') as f:
    mitigation_adversarial_imd = json.load(f)

with open(DATA_DIR / 'mitigation_results_adversarial_disability.json', 'r') as f:
    mitigation_adversarial_disability = json.load(f)

with open(DATA_DIR / 'mitigation_results_adversarial_age_band.json', 'r') as f:
    mitigation_adversarial_age = json.load(f)
...
: Prediction Window Design {#tbl-prediction-window}
...
%%| fig-cap: "Dual-branch LSTM-Based EWS Model Architecture"
...
: Fairness Threshold Interpretation {#tbl-threshold}
...
```{python}
#| label: tbl-audit
#| tbl-cap: Fairness Audit Results (95% CIs from 1,000 bootstrap samples)
#| output: asis
...
# 2. Build CI lookup from bootstrap results
ci_lookup = {}
for item in fairness_results_ci['summary']:
...
for item in fairness_results['summary']:
    ...
    ci = ci_lookup.get(attr, {})

    fairness_summary.append({
        'Attribute': attr,
        'SPD': item['SPD'],
        'SPD_Fair': spd_fair,
        'SPD_CI_low': ci.get('SPD_CI_low', None),
        'SPD_CI_high': ci.get('SPD_CI_high', None),
        'EOD': item['EOD'],
        'EOD_Fair': eod_fair,
        'EOD_CI_low': ci.get('EOD_CI_low', None),
        'EOD_CI_high': ci.get('EOD_CI_high', None),
        'EqOdds': item['EqOdds'],
...
markdown_output += "**95% CIs** from 1,000 bootstrap samples (test set, n=4,889)\n\n"

# Part B: The Table Header
markdown_output += "| Attribute | SPD | SPD 95% CI | EOD | EOD 95% CI | EqOdds | ABROCA | Status |\n"
markdown_output += "|:----------|:---:|:----------:|:---:|:----------:|:------:|:------:|:-------|\n"
...
for row in fairness_summary:
    ...
    spd_ci = fmt_ci(row['SPD_CI_low'], row['SPD_CI_high'])
    eod_ci = fmt_ci(row['EOD_CI_low'], row['EOD_CI_high'])

    # Flag wide CIs for age_band (small 55≤ subgroup)
    if row['Attribute'] == 'age_band':
        spd_ci += " *"
        eod_ci += " *"

    line = f"| {row['Attribute']} | {fmt(row['SPD'], row['SPD_Fair'])} | {spd_ci} | {fmt(row['EOD'], row['EOD_Fair'])} | {eod_ci} | {fmt(row['EqOdds'], row['EqOdds_Fair'], False)} | {fmt(row['ABROCA'], row['ABROCA_Fair'], False)} | {status} |\n"
    markdown_output += line

markdown_output += "\n\\* Wide CIs reflect small 55≤ subgroup (n=40); point estimates stable for all larger groups.\n"
...
```{python}
#| label: tbl-auc-by-group
#| tbl-cap: AUC by Demographic Group
#| output: asis
...
```{python}
#| label: fig-auc-by-group
#| fig-cap: "AUC comparison across demographic groups within each protected attribute"
...
We applied four mitigation techniques spanning all three mitigation families:

1. **Reweighting** (Pre-processing): Adjusts training sample weights so each protected-group/outcome combination receives equal representation ([AIF360](https://ai-fairness-360.org/){.external target="_blank"})
2. **Adversarial Debiasing** (In-processing): Extends the LSTM with an adversary branch that predicts the protected attribute from the model's learned representations; the main model is penalized when the adversary succeeds, forcing fair representations (PyTorch implementation)
3. **Threshold Optimization** (Post-processing): Sets group-specific classification thresholds to equalize true-positive rates
4. **Reject Option Classification** (Post-processing): Adjusts predictions near the decision boundary to reduce disparities ([AIF360](https://ai-fairness-360.org/){.external target="_blank"})
...
# 1. Prepare the data — best approach per attribute
mitigation_summary = []
...
# 2. Build the Markdown Table — shows best approach + adversarial for comparison
...
#| tbl-cap: Post-Mitigation Fairness Metrics (all three technique families evaluated)
#| output: asis

# Build the Markdown Table — includes best approach AND adversarial for each attribute
table_md = "| Attribute | Approach | AUC | SPD | EOD |\n"
table_md += "|:----------|:---------|:---:|:---:|:---:|\n"

# Pair each attribute with its adversarial result
adv_results = [
    ('Region', mitigation_adversarial_region),
    ('IMD Band', mitigation_adversarial_imd),
    ('Disability', mitigation_adversarial_disability),
    ('Age Band', mitigation_adversarial_age),
]
adv_lookup = {attr: res for attr, res in adv_results}

for _, row in mitigation_df.iterrows():
    # Best approach row
    table_md += f"| **{row['Attribute']}** | {row['Best_Approach']} | {row['Mitigated_AUC']:.3f} | {row['Mitigated_SPD']:+.3f} | {row['Mitigated_EOD']:+.3f} |\n"
    # Adversarial row
    adv = adv_lookup[row['Attribute']]['approaches']['adversarial']
    table_md += f"| | Adversarial (in-proc.) | {adv['auc']:.3f} | {adv['spd']:+.3f} | {adv['eod']:+.3f} |\n"

table_md += "\nAll approaches achieve |SPD| < 0.10 and |EOD| < 0.10. Baseline AUC = 0.889.\n"
...
| Any (in-proc.) | 457–3,649 (9–75%) | Adversarial Debiasing | Works across group sizes; all thresholds met |

: Mitigation Rationale {#tbl-approaches}
...
#### Why Adversarial Debiasing is a viable alternative:

- Adversarial debiasing extends the LSTM with a second "adversary" network that tries to predict the protected attribute from the model's internal representations
- During training, the main model is penalized when the adversary succeeds, forcing it to learn representations that are predictive of student risk but uninformative of group membership
- This in-processing approach achieved fairness thresholds (|SPD| < 0.10, |EOD| < 0.10) for all four unfair attributes
- While not selected as the best approach for any individual attribute, it demonstrates that in-processing methods provide a viable single-technique alternative that works across all group sizes
- The adversarial strength parameter (λ) was selected via grid search over {0.01, 0.1, 0.5, 1.0} for each attribute

**Key insight:** Pre-processing (reweighting) addresses representation imbalance at the source, post-processing (threshold optimization) works when the model already has adequate signal for all groups, and in-processing (adversarial debiasing) offers a universal alternative by learning fair representations directly.
...
: Most Problematic Intersections {#tbl-intersections}
...
: Selection Rate Extremes Comparison {#tbl-severe}
...
3. **Mitigation Effectiveness**
   - Threshold Optimization (post-processing): Best for Region, IMD, Age
   - Reweighting (pre-processing): Best for Disability
   - Adversarial Debiasing (in-processing): Viable alternative for all attributes
...
- Consider Adversarial Debiasing as a single-technique alternative across all attributes
...
- Explore longitudinal fairness monitoring (whether fairness degrades over academic cohorts)
...
: 10-Week Prediction Window Trade-offs {#tbl-trade-offs}
...
- **Uncertainty:** Bootstrap 95% CIs for SPD and EOD are reported in the fairness audit table (1,000 iterations). Most metrics show stable intervals; the notably wide CIs for age_band reflect the small 55≤ subgroup (n=40). AUC and post-mitigation metrics remain point estimates.
...
3. **Effective Bias Mitigation:** Reduced disparities using all three mitigation families (pre-processing reweighting, in-processing adversarial debiasing, and post-processing threshold optimization) while maintaining model performance
